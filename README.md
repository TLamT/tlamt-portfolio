...
- **資料自動同步** — 專案清單直接由 **GitHub API** 攞，一個鐘 revalidate 一次，唔使手動維護
- **繁體中文內容** — 全站廣東話/中文
...
- **專案列表**：`https://api.github.com/users/TLamT/repos`（跳過 fork / 空 repo）
- **README**：`https://raw.githubusercontent.com/TLamT/<repo>/<branch>/README.md`
- **中文資料**：每個專案嘅中文名、vibe、描述寫死喺 `src/lib/projects.ts`，方便自己執。

> GitHub API 冇 token 嘅話每小時限 60 次。由於網站有 1 小時快取，正常使用綽綽有餘；想加額度可以喺環境變數加 `GITHUB_TOKEN`（`src/lib/github.ts` 依家未用到，需要的話可以加返）。
